...
public class FileServiceImpl implements FileService {
    ...
    public FileServiceImpl(FileUtil fileUtil, FileMapper fileMapper, WorkMapper workMapper, AdminMapper adminMapper, TeamMapper teamMapper, UserMapper userMapper) {
    ...
    public void exportComInfo(HttpServletResponse response, Long comId) {
        try {
            ...
            // 获取excel数据
            List<List<String>> dataList = dataList(comId, competition);
            // 这里需要设置不关闭流
     ...
     */
    private List<List<String>> dataList(Long comId, Competition competition) {
        ...
        for (Work work : workList) {
            ...
            // 比赛名称
            tempData.add(competition.getName());
            // 队长学号
            String captain = work.getUserCode();
            teamQueryWrapper.eq("com_id", comId).eq("captain", captain);
            Team team = teamMapper.selectOne(teamQueryWrapper);
            teamQueryWrapper.clear();
            // 队伍名称
            tempData.add(team.getName());
            // 队长名字
            userQueryWrapper.eq("code", captain).select("name");
            User user = userMapper.selectOne(userQueryWrapper);
            userQueryWrapper.clear();
            if (user == null) {
                tempData.add("（空）");
            } else {
                tempData.add(user.getName());
            }
            // 查找队员
            JSONArray members = JSONArray.parseArray(team.getMember());
            // 判空? 似乎也不需要判空了
            List<Map> memMap = members.toJavaList(Map.class);
            for (Map map : memMap) {
                String memName = map.get("name").toString();
